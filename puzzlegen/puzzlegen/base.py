...
class PuzzleEnv(Env):
    ...
    def step(self, a):
        """Step the environment and convert to gymnasium format."""
        if self.needs_reset:
        ...
        # _step returns (obs, reward, done, info) - convert to gymnasium format
        obs, reward, done, info = self._step(a)
        # Convert done to terminated/truncated (gymnasium format)
        terminated = done
        truncated = False
